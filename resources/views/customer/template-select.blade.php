            ...
            <div class="selected-inputs">
                ...
                @foreach($products as $productImage)
                    ...
                    <form action="{{ route('filter.product') }}" method="POST">
                        ...
                        <div class="template-input wish-select  @if($loop->first) selected  @endif " onclick="this.closest('form').submit()"  id="product_{{ $productImage->id }}" 
                        data-category-id="{{ $productImage->id }}">
                            <div class="image">
                            @if($productImage->is_featured_image == 1)
                            <img class="premium-icon" src="{{url('public/auth/img/premium.svg')}}" alt="premium" />
                            @endif
                            <img class="tempimg " src="{{$productImage->product_image}}"  alt="{{$category->category_name}}" />
                            </div>
                            <h4>Happy {{$category->category_name}} Celebration</h4>
...
<script>
    $(document).on('click','.wish-select',function(){
      ...
      document.querySelectorAll('.wish-select').forEach(function(element) {
        ...
        document.getElementById('product_' + categoryId).classList.add('selected');
