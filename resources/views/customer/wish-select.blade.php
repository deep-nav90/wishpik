            ...
            <div class="selected-inputs">
                ...
                @foreach($categories as $category_list)
                    ...
                    <form action="{{ route('filter.category') }}" method="POST">
                        @csrf
                        <input type="hidden" name="category_id" value="{{ $category_list->id }}">
                        <div class="wish-input @if($loop->first) selected-input @endif"  id="category_{{ $category_list->id }}" 
                        data-category-id="{{ $category_list->id }}" onclick="this.closest('form').submit()" >
                            <img src="{{ $category_list->category_image }}" alt="{{ $category_list->category_name }}" />
                            <span>{{ $category_list->category_name }}</span>
                        </div>
                    </form>
                    </div>
                    @endforeach
...
<script>
    $(document).on('click','.wish-input',function(){

        let cag=this.id
        console.log('fdfdfdf',cag)
      let mar=  cag.split('_');
      let categoryId=mar[1]
   
      document.querySelectorAll('.wish-input').forEach(function(element) {
            element.classList.remove('selected-input');
        });

        // Add the 'selected-input' class to the clicked category
        document.getElementById('category_' + categoryId).classList.add('selected-input');
